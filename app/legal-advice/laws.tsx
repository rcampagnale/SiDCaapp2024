...
export default function Laws({setActionType}:HandleAdviceStatus){
    ...
    useEffect(()=>{
        const getData=async()=>{
            try {
                const res=(await getDocs(data)).docs.filter(item=>item.data().categoria === "leyes")
                setLaws(res)
            } catch (error) {
                console.log(error)
            }finally {
